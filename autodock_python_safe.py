...
def prepareprot_scaffold(preload, presave, receptor, postload, postsave, algchoice):
    ...
    canproceed = True #####Newly added for safe version---------
    if canproceed:
        # cmd.remove('resn HOH')
        # #cmd.h_add(selection='acceptors or donors')
        # cmd.save(presave+receptor+postsave)
        readlist = []                                                    #####THIS IS NEWLY ADDED AS WELL FOR SAFE VERION------------
        with open(preload+receptor+postload, 'r+') as file_in:
            for line in file_in:
                readlist.append(line[:-1]) #escape newline character
                with open(presave+receptor+postsave, "w") as file_out:
                        for item in reclist:
                            file_out.write("%s\n" % item)
